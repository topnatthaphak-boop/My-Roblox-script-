...
-- [[ ปุ่ม Fly.T&D - ผลงานชิ้นเอกของทีมเรา ]]
OthersTab:CreateButton({
   Name = "Fly.T&D (ระบบลอยตัว/มุดดิน/ความเร็ว)",
   Callback = function()
      local rPart = char():WaitForChild("HumanoidRootPart")
      local hmd = hum()
      local isLocked, isNoc, isSpd = false, false, false
      local fForce, nConn = nil, nil

      local sg = Instance.new("ScreenGui", player.PlayerGui)
      sg.Name = "TD_Movement_Final"
      local frame = Instance.new("Frame", sg)
      frame.Size, frame.Position = UDim2.new(0,160,0,120), UDim2.new(0,50,0.5,-60)
      frame.BackgroundColor3, frame.Active, frame.Draggable = Color3.fromRGB(15,15,15), true, true
      Instance.new("UIGridLayout", frame).CellSize = UDim2.new(0,50,0,35)

      local function btn(txt, ord, col)
         local b = Instance.new("TextButton", frame)
         b.Text, b.LayoutOrder, b.BackgroundColor3 = txt, ord, col
         b.TextColor3, b.Font, b.TextSize, b.BorderSizePixel = Color3.new(1,1,1), Enum.Font.GothamBold, 10, 0
         return b
      end

      local u, title, d = btn("UP",1,Color3.fromRGB(140,80,250)), btn("gui by T&D",2,Color3.fromRGB(100,50,200)), btn("DOWN",3,Color3.fromRGB(140,80,250))
      local l, nc, sp = btn("L (OFF)",4,Color3.fromRGB(60,20,120)), btn("Clip(OFF)",5,Color3.fromRGB(40,40,40)), btn("SPD(OFF)",6,Color3.fromRGB(40,40,40))

      l.MouseButton1Click:Connect(function()
         isLocked = not isLocked
         if isLocked then
            fForce = Instance.new("BodyVelocity", rPart)
            fForce.MaxForce, fForce.Velocity = Vector3.new(0,math.huge,0), Vector3.new(0,0,0)
            l.Text, l.BackgroundColor3 = "L (ON)", Color3.fromRGB(180,50,255)
         else
            if fForce then fForce:Destroy() fForce = nil end
            l.Text, l.BackgroundColor3 = "L (OFF)", Color3.fromRGB(60,20,120)
         end
      end)
      nc.MouseButton1Click:Connect(function()
         ...
         if isNoc then
            ...
            nc.Text, nc.BackgroundColor3 = "Noclipping", Color3.fromRGB(200,0,0)
         else
            if nConn then nConn:Disconnect() nConn = nil end
            nc.Text, nc.BackgroundColor3 = "Clip(OFF)", Color3.fromRGB(40,40,40)
         end
      end)
      sp.MouseButton1Click:Connect(function()
         isSpd = not isSpd
         if hmd then hmd.WalkSpeed = isSpd and 100 or 16 end
         sp.Text, sp.BackgroundColor3 = isSpd and "SPD (100)" or "SPD (OFF)", isSpd and Color3.fromRGB(0,180,255) or Color3.fromRGB(40,40,40)
      end)
      u.MouseButton1Click:Connect(function() if isLocked then rPart.CFrame *= CFrame.new(0,5,0) end end)
      d.MouseButton1Click:Connect(function() if isLocked then rPart.CFrame *= CFrame.new(0,-5,0) end end)
      Rayfield:Notify({Title = "T&D Team", Content = "Fly.T&D ระบบพร้อมใช้งาน!", Duration = 3})
   end
})
